...
public class OpenApiCoverageTests
{
    ...
    {
        var sourceRoot = Path.Combine(sdkRoot, "src", "Commerce");
        var paths = new HashSet<string>(StringComparer.Ordinal);
    ...
    {
        var overridePath = Environment.GetEnvironmentVariable("COMMERCE_OPENAPI_SPEC");
        ...
        throw new FileNotFoundException("Could not find OpenAPI spec. Set COMMERCE_OPENAPI_SPEC or run tests from sdks/dotnet.", defaultPath);
    }
    ...
    private static string FindSdkRoot()
    {
        foreach (var start in SearchStarts())
        {
            for (var current = new DirectoryInfo(start); current != null; current = current.Parent)
            {
                var candidate = Path.Combine(current.FullName, "src", "Commerce", "Commerce.csproj");
                if (File.Exists(candidate))
                {
