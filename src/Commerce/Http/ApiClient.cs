...
internal class ApiClient : IDisposable
{
    ...
    public ApiClient(string apiKey, string baseUrl, TimeSpan timeout, HttpClient? httpClient = null)
    {
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("zebo-commerce-sdk-dotnet/0.1.0");
        _httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
    }

    public async Task<CommerceResponse> PostAsync(string path, object? payload = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new CommerceTimeoutException("Request timed out", ex);
        }
        catch (Exception ex) when (ex is HttpRequestException or TaskCanceledException)
        {
            throw new CommerceNetworkException("Network request failed", ex);
        }
        ...
        return new CommerceResponse(parsed);
    }

    public async Task<CommerceResponse> PostWithHeadersAsync(
        string path,
        object? payload = null,
    ...
    public async Task<CommerceResponse> PostMultipartAsync(
        string pathOrUrl,
    ...
    private async Task<CommerceResponse> SendForJsonAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        HttpResponseMessage response;
        try
        {
    ...
    private void HandleError(HttpResponseMessage response, string rawBody, JsonNode? parsed)
    {
        ...
        if (status == 429)
        {
            ...
            throw new CommerceRateLimitException(
                message,
                status,
                code,
                type,
